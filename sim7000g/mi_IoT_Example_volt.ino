...
Adafruit_FONA_LTE fona = Adafruit_FONA_LTE();
  ...
  #define AIO_USERNAME    "pelempito"
  #define AIO_KEY         "aio_OygC97binHhGB0JE8vfZqgYwCiHQ"
...
//  }
  ...
  */
  ...
  // fona.setNetworkSettings(F("internet.ideasclaro.com.do"));
  ...
  // fona.setPreferredMode(2); // Auto
  fona.setPreferredMode(13); // GSM only
  // fona.setPreferredMode(38); // LTE only
  // fona.setPreferredMode(51); // GSM and LTE only  
  ...
  */
  ...
  while (counter < 150 && !fona.enableGPS(true)) {
    ...
    if (counter == 150) { // 150 loops equals 5 minutes
        ...
        fona.enableGPS(true);
        delay(2000);
        Serial.println(F("Tried to enable GPS while trying to turn on GPS"));
    }
  ...
  Serial.println(F("Turned on GPS!"));
    ...
    // Turn on GPRS
    counter = 0;
    while (counter < maxCounter && !fona.enableGPRS(true)) {
      Serial.print(F("Failed to enable GPRS, retrying... (GPRS loop), attempt number ")); Serial.println(counter);
      counter++; // Increment counter
      if (counter == maxCounter) {
        fona.enableGPRS(false);
        Serial.println(F("Tried to disable GPRS (GPRS loop)"));
        delay(2000);
        fona.enableGPRS(true);
        delay(2000);
        Serial.println(F("Tried to enable GPRS (GPRS loop)"));
        }
      delay(2000);
    }
    /* ORIGINAL APPROACH, to be deleted if above code works
...
void loop() {
  ...
  // From geofis: "keep retrying", or "keep waiting" until a connection is made?
  // This is why I added moduleSetup. See: https://github.com/botletics/SIM7000-LTE-Shield/issues/238
  while (!netStatus()) {
    Serial.println(F("Failed to connect to cell network, retrying..."));
    moduleSetup(); // Setup the module
    delay(5000); // Retry every 5s. I increased the delay to allow the full setup to run
  }
  ...
  battLevel = battery.chargePercentage(); // Get battery % of charge
...
//  float temperature = tempC; // Select what unit you want to use for this example
  ...
  while (counter < 150 && (!fona.enableGPS(true) || !fona.getGPS(&latitude, &longitude, &speed_kph, &heading, &altitude))) {
    ...
    if (counter == 150) { // 150 loops equals 5 minutes
        ...
        fona.enableGPS(true);
        delay(2000);
        Serial.println(F("Tried to enable GPS while trying to get location"));
    }
    delay(2000); // Retry every 2s
  }
  ...
  */
  ...
  // If the shield was already on, no need to re-enable. WELL, NOT SO SURE
...
//  dtostrf(temperature, 1, 2, tempBuff); // float_val, min_width, digits_after_decimal, char_buffer
  ...
  #else
    sprintf(URL, "http://dweet.io/dweet/for/%s?lat=%s&long=%s&batt=%s", imei, latBuff, longBuff,
            battBuff);    
...
//            speedBuff, headBuff, altBuff, tempBuff, battBuff);
    while (counter < maxCounter && !fona.postData("GET", URL)) {
      Serial.println(F("Failed to post data, retrying..."));
      counter++; // Increment counter
      if (counter == maxCounter) {
        fona.enableGPRS(false);
        Serial.println(F("Tried to disable GPRS"));
        delay(2000);
        fona.enableGPRS(true);
        delay(2000);
        Serial.println(F("Tried to enable GPRS"));
        }
      delay(1000);
    }
  #endif
...
void moduleSetup() {
  ...
  if (! fona.begin(fonaSS)) {
    ...
    while (!fona.begin(fonaSS)) {
      Serial.println(F("Couldn't find FONA, retrying..."));
      delay(5000);
      }
